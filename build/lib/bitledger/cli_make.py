...
def resolve_encoding_plan(
    ...
    if ns.sf is not None:
        ...
        found = find_smallest_sf(amt, dp, pair, S, lo, hi)
        if found is None:
            return None, (
                2,
        ...
        sf_banner = (
            f"(Smallest SF in range {lo}..{min(hi, len(encoder.SCALING_FACTORS) - 1)}: {sf})"
        )
...
def add_make_arguments(p: argparse.ArgumentParser) -> None:
    p.add_argument("--profile", help="Profile JSON (Layer2 dp/split/currency defaults)")
    p.add_argument(
        "--amount",
        required=True,
        metavar="DECIMAL",
    ...
    p.add_argument(
        ...
        type=int,
        default=17,
        help="When searching, maximum SF index (default 17; table-limited)",
    )
    p.add_argument(
        "--emit-l2",
