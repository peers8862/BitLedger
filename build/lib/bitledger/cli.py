...
from bitledger.cli_make import add_make_arguments, cmd_check_amount, cmd_make, parse_amount_string
...
def cmd_encode(ns: argparse.Namespace) -> int:
    ...
    l2 = Layer2Config()
    if ns.profile:
        l1, l2 = load_profile(Path(ns.profile))
    try:
    ...
    if ns.amount is not None:
        ...
        try:
            _N, A, r, rf, rd = encoder.encode_value(
                ...
                l2.decimal_position & 7,
                ns.account_pair & 0xF,
                l2.optimal_split & 0xF,
            )
        except EncoderError as e:
            print(str(e), file=sys.stderr)
            return 2
...
def cmd_simulate(ns: argparse.Namespace) -> int:
    l1, l2 = load_profile(Path(ns.profile)) if ns.profile else (Layer1Config(), Layer2Config())
    rec = TransactionRecord(
        multiplicand=1,
        multiplier=197,
        rounding_flag=False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    pe.add_argument("--profile", help="Profile JSON from setup")
    ...
    pe.add_argument("--sf", type=int, help="Layer2 scaling_factor_index 0-127")
    ...
    psf = sub.add_parser(
        "suggest-sf",
        help="Alias of make — same output (smallest SF in range unless --sf is set)",
    )
